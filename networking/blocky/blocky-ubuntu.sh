...
SERVICE_DIR="/root/docker/blocky"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(blocky)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Blocky"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^blocky$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs blocky"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Blocky to be ready on port 4002..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:4002 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 4002 is responding (HTTP $STATUS) — Blocky is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 4002 2>/dev/null; then
        warn "Port 4002 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 4002 is NOT responding yet."
        docker logs --tail 20 blocky 2>&1 || true
    fi
    warn "Check logs: docker logs blocky"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🛡️   Blocky (DNS ad-blocker):  DNS on $SERVER_IP:5354"
echo "  ║  📊  API/metrics: http://$SERVER_IP:4002"
echo "  ║  🔧  Edit blocklists in $SERVICE_DIR/config/config.yml"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
