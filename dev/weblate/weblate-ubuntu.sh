...
read -rp "  Admin email [admin@weblate.local]: " ADMIN_EMAIL
ADMIN_EMAIL="${ADMIN_EMAIL:-admin@weblate.local}"
info "Admin: $ADMIN_EMAIL"

section "Step 5: Cleaning Up Existing Containers"
for cname in weblate weblate-postgres weblate-redis; do
    EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E "^${cname}$" || true)
    [ -n "$EXISTING" ] && warn "Removing $cname..." && docker rm -f "$cname" 2>/dev/null || true
done

section "Step 6: Preparing Directory"
APP_DIR="/root/docker/weblate"
...
section "Step 8: Starting Weblate"
if docker compose version &> /dev/null; then
...
info "Waiting for Weblate to be ready..."
for i in $(seq 1 18); do
    if curl -sf --max-time 5 http://127.0.0.1:8003 &>/dev/null; then
        info "Weblate is ready. ✅"
        break
    fi
