...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║        Navidrome Music Server Auto-Installer     ║"
...
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E 'navidrome' || true)
if [ -n "$EXISTING" ]; then
    warn "Removing existing containers..."
    echo "$EXISTING" | xargs docker rm -f 2>/dev/null || true
else
    info "No existing Navidrome containers found."
fi
docker network prune -f &>/dev/null || true

section "Step 5: Preparing Directory"
NAVIDROME_DIR="/root/docker/navidrome"
...
cat > "$NAVIDROME_DIR/docker-compose.yml" <<EOF
services:
  navidrome:
...
echo "  ║                                                      ║"
echo "  ║  📂  Add your music files to:                       ║"
echo "  ║      $NAVIDROME_DIR/music"
echo "  ║                                                      ║"
