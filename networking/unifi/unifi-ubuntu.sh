#!/bin/bash
# ============================================================
#   UniFi Network Application Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   UniFi Network Application Auto-Installer"
...
SERVICE_DIR="/root/docker/unifi"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(unifi|unifi-mongo)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/config" "$SERVICE_DIR/mongo"
...
cat > "$SERVICE_DIR/init-mongo.js" <<MONGOEOF
...
MONGOEOF
info "Mongo init script created."
...
section "Step 7: Starting UniFi Network Application"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^unifi$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs unifi"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for UniFi Network Application to be ready on port 8443..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -k -o /dev/null -w "%{http_code}" --max-time 5 https://127.0.0.1:8443 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8443 is responding (HTTP $STATUS) — UniFi Network Application is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8443 2>/dev/null; then
        warn "Port 8443 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8443 is NOT responding yet."
        docker logs --tail 20 unifi 2>&1 || true
    fi
    warn "Check logs: docker logs unifi"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📶  UniFi Network:  https://$SERVER_IP:8443"
echo "  ║  🔧  Create your admin account in the setup wizard."
echo "  ║  🛰️   Device adoption/inform: http://$SERVER_IP:8080/inform"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
