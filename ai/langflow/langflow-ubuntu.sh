...
SERVICE_DIR="/root/docker/langflow"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(langflow)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Langflow"
warn "Langflow is a large image — first start takes a few minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^langflow$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs langflow"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Langflow to be ready on port 7860..."
HEALTH_OK=0
for i in $(seq 1 48); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:7860 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 7860 is responding (HTTP $STATUS) — Langflow is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 7860 2>/dev/null; then
        warn "Port 7860 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 7860 is NOT responding yet."
        docker logs --tail 20 langflow 2>&1 || true
    fi
    warn "Check logs: docker logs langflow"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🧩  Langflow (visual LLM flows):  http://$SERVER_IP:7860"
echo "  ║  🔗  Point OpenAI-style components at your Ollama/LiteLLM scripts for a local stack."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
