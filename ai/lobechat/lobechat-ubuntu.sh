...
SERVICE_DIR="/root/docker/lobechat"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(lobechat)$' || true)
if [ -n "$EXISTING" ]; then
...
ACCESS_CODE=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 16)
...
section "Step 7: Starting LobeChat"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^lobechat$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs lobechat"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for LobeChat to be ready on port 3211..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3211 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3211 is responding (HTTP $STATUS) — LobeChat is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3211 2>/dev/null; then
        warn "Port 3211 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3211 is NOT responding yet."
        docker logs --tail 20 lobechat 2>&1 || true
    fi
    warn "Check logs: docker logs lobechat"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  💬  LobeChat:  http://$SERVER_IP:3211"
echo "  ║  🔑  Access code: $ACCESS_CODE"
echo "  ║  🔗  Pre-wired for local Ollama (port 11434) — add OpenAI/Claude keys in settings."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
