...
SERVICE_DIR="/root/docker/shynet"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(shynet|shynet-db)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Shynet"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^shynet$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs shynet"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Shynet to be ready on port 8233..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8233 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8233 is responding (HTTP $STATUS) — Shynet is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8233 2>/dev/null; then
        warn "Port 8233 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8233 is NOT responding yet."
        docker logs --tail 20 shynet 2>&1 || true
    fi
    warn "Check logs: docker logs shynet"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📈  Shynet (privacy analytics):  http://$SERVER_IP:8233"
echo "  ║  ➕  Create the admin:"
echo "  ║      docker exec -it shynet ./manage.py registeradmin your@email.com"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
