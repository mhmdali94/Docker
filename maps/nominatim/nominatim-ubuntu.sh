...
SERVICE_DIR="/root/docker/nominatim"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(nominatim)$' || true)
if [ -n "$EXISTING" ]; then
...
info "Nominatim will import this region on first start."
...
section "Step 7: Starting Nominatim"
warn "First start downloads and imports the region — small regions take a few minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^nominatim$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs nominatim"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Nominatim to be ready on port 8318..."
HEALTH_OK=0
for i in $(seq 1 60); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8318/status 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8318 is responding (HTTP $STATUS) — Nominatim is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8318 2>/dev/null; then
        warn "Port 8318 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8318 is NOT responding yet."
        docker logs --tail 20 nominatim 2>&1 || true
    fi
    warn "Check logs: docker logs nominatim"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📍  Nominatim (geocoding):  http://$SERVER_IP:8318"
echo "  ║  🔗  Try: http://$SERVER_IP:8318/search?q=casino+monaco&format=json"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
