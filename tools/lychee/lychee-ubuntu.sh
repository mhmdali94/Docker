...
SERVICE_DIR="/root/docker/lychee"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(lychee)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/conf" "$SERVICE_DIR/uploads" "$SERVICE_DIR/sym" "$SERVICE_DIR/logs"
...
section "Step 7: Starting Lychee"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^lychee$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs lychee"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Lychee to be ready on port 8184..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8184 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8184 is responding (HTTP $STATUS) — Lychee is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8184 2>/dev/null; then
        warn "Port 8184 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8184 is NOT responding yet."
        docker logs --tail 20 lychee 2>&1 || true
    fi
    warn "Check logs: docker logs lychee"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📷  Lychee (photo albums):  http://$SERVER_IP:8184"
