...
SERVICE_DIR="/root/docker/gitness"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(gitness)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  gitness:
    ...
    ports:
      - "3047:3000"
    environment:
      GITNESS_URL_BASE: http://$SERVER_IP:3047
    volumes:
      - ./data:/data
...
section "Step 7: Starting Gitness"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^gitness$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs gitness"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Gitness to be ready on port 3047..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3047 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3047 is responding (HTTP $STATUS) — Gitness is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3047 2>/dev/null; then
        warn "Port 3047 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3047 is NOT responding yet."
        docker logs --tail 20 gitness 2>&1 || true
    fi
    warn "Check logs: docker logs gitness"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🧑‍💻  Gitness (Git + CI pipelines):  http://$SERVER_IP:3047"
echo "  ║  🔧  Sign up the first user — it becomes the admin."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
