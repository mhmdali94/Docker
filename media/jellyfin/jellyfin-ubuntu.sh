...
if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qE '^jellyfin$'; then
    warn "Removing existing container: jellyfin"
...
info "Add your media files to: $JF_DIR/media"
...
for i in $(seq 1 12); do
    if curl -s --max-time 5 http://127.0.0.1:8096/health &>/dev/null; then
...
[ "$HEALTH_OK" -eq 0 ] && warn "Jellyfin may still be initializing. Check: docker logs jellyfin"

section "Step 10: Opening Firewall Ports"
if command -v ufw &> /dev/null; then
    ufw allow 8096/tcp
    ufw allow 8920/tcp
    info "UFW: ports 8096/tcp and 8920/tcp opened."
else
...
echo "  ║  📁  Add media to: $JF_DIR/media                   ║"
echo "  ║                                                      ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️            ║"
echo "  ║       Made by: Mohammed Ali Elshikh                 ║"
echo "  ║       prismatechwork.com                            ║"
