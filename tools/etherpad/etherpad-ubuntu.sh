...
SERVICE_DIR="/root/docker/etherpad"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(etherpad)$' || true)
if [ -n "$EXISTING" ]; then
...
chown -R 5001:0 "$SERVICE_DIR/data"
...
section "Step 7: Starting Etherpad"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^etherpad$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs etherpad"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Etherpad to be ready on port 9004..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:9004 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 9004 is responding (HTTP $STATUS) — Etherpad is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 9004 2>/dev/null; then
        warn "Port 9004 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 9004 is NOT responding yet."
        docker logs --tail 20 etherpad 2>&1 || true
    fi
    warn "Check logs: docker logs etherpad"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ✍️   Etherpad (collaborative docs):  http://$SERVER_IP:9004"
echo "  ║  🔑  Admin UI: http://$SERVER_IP:9004/admin  (admin / $ADMIN_PASS)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
