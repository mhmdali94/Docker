...
SERVICE_DIR="/root/docker/yacht"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(yacht)$' || true)
if [ -n "$EXISTING" ]; then
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^yacht$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs yacht"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Yacht to be ready on port 8161..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8161 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8161 is responding (HTTP $STATUS) — Yacht is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8161 2>/dev/null; then
        warn "Port 8161 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8161 is NOT responding yet."
        docker logs --tail 20 yacht 2>&1 || true
    fi
    warn "Check logs: docker logs yacht"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⛵  Yacht (container management):  http://$SERVER_IP:8161"
echo "  ║  🔑  Default login: admin@yacht.local / pass  (change it!)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
