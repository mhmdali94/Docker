...
SERVICE_DIR="/root/docker/budibase"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(budibase)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Budibase"
warn "Budibase is an all-in-one image — first start takes 2-3 minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^budibase$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs budibase"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Budibase to be ready on port 8159..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8159 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8159 is responding (HTTP $STATUS) — Budibase is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8159 2>/dev/null; then
        warn "Port 8159 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8159 is NOT responding yet."
        docker logs --tail 20 budibase 2>&1 || true
    fi
    warn "Check logs: docker logs budibase"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🧱  Budibase (low-code apps):  http://$SERVER_IP:8159"
