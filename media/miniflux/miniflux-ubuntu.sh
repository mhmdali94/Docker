...
SERVICE_DIR="/root/docker/miniflux"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(miniflux|miniflux-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  miniflux:
    ...
    ports:
      - "8215:8080"
    environment:
      DATABASE_URL: postgres://miniflux:$DB_PASS@miniflux-db/miniflux?sslmode=disable
      RUN_MIGRATIONS: 1
      CREATE_ADMIN: 1
...
section "Step 7: Starting Miniflux"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^miniflux$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs miniflux"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Miniflux to be ready on port 8215..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8215 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8215 is responding (HTTP $STATUS) — Miniflux is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8215 2>/dev/null; then
        warn "Port 8215 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8215 is NOT responding yet."
        docker logs --tail 20 miniflux 2>&1 || true
    fi
    warn "Check logs: docker logs miniflux"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📰  Miniflux (minimalist RSS reader):  http://$SERVER_IP:8215"
