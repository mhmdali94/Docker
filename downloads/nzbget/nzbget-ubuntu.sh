...
SERVICE_DIR="/root/docker/nzbget"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(nzbget)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting NZBGet"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^nzbget$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs nzbget"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for NZBGet to be ready on port 6789..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:6789 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 6789 is responding (HTTP $STATUS) — NZBGet is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 6789 2>/dev/null; then
        warn "Port 6789 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 6789 is NOT responding yet."
        docker logs --tail 20 nzbget 2>&1 || true
    fi
    warn "Check logs: docker logs nzbget"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📰  NZBGet:  http://$SERVER_IP:6789"
echo "  ║  🔑  Default login: nzbget / tegbzn6789  (change it!)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
