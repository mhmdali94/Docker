...
SERVICE_DIR="/root/docker/valkey"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(valkey)$' || true)
if [ -n "$EXISTING" ]; then
...
VALKEY_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 24)
info "Password: $VALKEY_PASS"
...
services:
  valkey:
    ...
    command: valkey-server --requirepass $VALKEY_PASS --appendonly yes
    ports:
      - "6381:6379"
    volumes:
      - ./data:/data
...
section "Step 7: Starting Valkey"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^valkey$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs valkey"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Valkey on TCP port 6381..."
HEALTH_OK=0
for i in $(seq 1 24); do
    if nc -z 127.0.0.1 6381 2>/dev/null; then
        info "Port 6381 is open — Valkey is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 6381 not responding yet. Check: docker logs valkey"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔑  Valkey (Redis successor):  $SERVER_IP:6381"
echo "  ║  🔑  Password: $VALKEY_PASS"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
