...
SERVICE_DIR="/root/docker/mailpit"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(mailpit)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Mailpit"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^mailpit$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs mailpit"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Mailpit to be ready on port 8025..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8025 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8025 is responding (HTTP $STATUS) — Mailpit is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8025 2>/dev/null; then
        warn "Port 8025 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8025 is NOT responding yet."
        docker logs --tail 20 mailpit 2>&1 || true
    fi
    warn "Check logs: docker logs mailpit"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📨  Mailpit (dev SMTP catcher):  http://$SERVER_IP:8025"
echo "  ║  🔧  Point your apps at SMTP $SERVER_IP:1025 — all mail is captured here."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
