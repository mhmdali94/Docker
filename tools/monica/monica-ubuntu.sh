...
for cname in monica monica-db; do
...
MN_DIR="/root/docker/monica"
if [ -d "$MN_DIR" ]; then
    warn "Removing old directory $MN_DIR..."
    rm -rf "$MN_DIR"
fi
mkdir -p "$MN_DIR/storage"
cd "$MN_DIR" || error "Cannot navigate to $MN_DIR"
info "Directory ready: $MN_DIR"
...
info "Register your account on first visit at http://$SERVER_IP:8096"

cat > "$MN_DIR/docker-compose.yml" <<EOF
services:
  monica-db:
    image: mariadb:10.11
...
section "Step 7: Starting Monica"
if docker compose version &> /dev/null; then
    docker compose up -d
else
    docker-compose up -d
...
sleep 15
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^monica$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs monica"
else
...
if [ "$HEALTH_OK" -eq 0 ]; then
    warn "Monica may still be initializing. Check: docker logs monica"
fi

section "Step 10: Opening Firewall Port 8096"
if command -v ufw &> /dev/null; then
    ufw allow 8096/tcp
    info "UFW: port 8096/tcp opened."
else
...
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║                                                      ║"
echo "  ║  🌐  Open Monica in your browser:                ║"
