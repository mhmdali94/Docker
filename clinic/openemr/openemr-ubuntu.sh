...
echo "  ║                                                      ║"
echo "  ║  ⏳  First startup takes 3-5 minutes while          ║"
echo "  ║     OpenEMR initializes and migrates the database.  ║"
echo "  ║                                                      ║"
...
for C in openemr openemr-db; do
...
OE_DIR="/root/docker/openemr"
if [ -d "$OE_DIR" ]; then
    warn "Removing old directory $OE_DIR..."
    rm -rf "$OE_DIR"
fi
mkdir -p "$OE_DIR/logs" "$OE_DIR/sites"
cd "$OE_DIR" || error "Cannot navigate to $OE_DIR"
info "Directory ready: $OE_DIR"
...
cat > "$OE_DIR/docker-compose.yml" <<EOF
services:
  openemr-db:
    image: mariadb:10.6
...
section "Step 7: Starting OpenEMR"
MAX_RETRIES=3
...
sleep 15
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^openemr$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs openemr"
else
...
for i in $(seq 1 18); do
    ...
    echo -n "  Attempt $i/18 — waiting 20s (DB init in progress)..."
    sleep 20
    echo " retrying"
done
if [ "$HEALTH_OK" -eq 0 ]; then
    warn "OpenEMR may still be initializing. Check: docker logs openemr"
fi

section "Step 10: Opening Firewall Port 8124"
if command -v ufw &> /dev/null; then
    ufw allow 8124/tcp
    info "UFW: port 8124/tcp opened."
else
...
echo "  ║                                                      ║"
echo "  ║  🌐  OpenEMR URL:                                  ║"
echo "  ║      http://$SERVER_IP:8124"
echo "  ║                                                      ║"
echo "  ║  🔑  Login Credentials (save these!):              ║"
echo "  ║      Username : admin"
echo "  ║      Password : $ADMIN_PASS"
echo "  ║                                                      ║"
echo "  ║  ⏳  If not ready, wait 3-5 min for DB migration.  ║"
echo "  ║      Monitor: docker logs openemr                   ║"
echo "  ║                                                      ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️            ║"
echo "  ║       Made by: Mohammed Ali Elshikh                 ║"
echo "  ║       prismatechwork.com                            ║"
