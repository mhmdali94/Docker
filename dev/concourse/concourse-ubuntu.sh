...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Concourse CI Auto-Installer"
...
SERVICE_DIR="/root/docker/concourse"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(concourse|concourse-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  concourse:
    ...
    environment:
      ...
      CONCOURSE_EXTERNAL_URL: http://$SERVER_IP:8244
      CONCOURSE_ADD_LOCAL_USER: admin:$ADMIN_PASS
...
section "Step 7: Starting Concourse CI"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^concourse$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs concourse"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Concourse CI to be ready on port 8244..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8244 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8244 is responding (HTTP $STATUS) — Concourse CI is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8244 2>/dev/null; then
        warn "Port 8244 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8244 is NOT responding yet."
        docker logs --tail 20 concourse 2>&1 || true
    fi
    warn "Check logs: docker logs concourse"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ✈️   Concourse CI:  http://$SERVER_IP:8244"
