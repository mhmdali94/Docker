...
SERVICE_DIR="/root/docker/komodo"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(komodo-core|komodo-mongo|komodo-periphery)$' || true)
if [ -n "$EXISTING" ]; then
...
PASSKEY=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 32)
...
services:
  ...
  komodo-core:
    ...
    ports:
      - "9120:9120"
    environment:
      KOMODO_HOST: http://$SERVER_IP:9120
      KOMODO_TITLE: Komodo
...
section "Step 7: Starting Komodo"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^komodo-core$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs komodo-core"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Komodo to be ready on port 9120..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:9120 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 9120 is responding (HTTP $STATUS) — Komodo is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 9120 2>/dev/null; then
        warn "Port 9120 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 9120 is NOT responding yet."
        docker logs --tail 20 komodo-core 2>&1 || true
    fi
    warn "Check logs: docker logs komodo-core"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🦎  Komodo (Docker build & deploy):  http://$SERVER_IP:9120"
echo "  ║  🔧  Sign up the first user on first visit (becomes admin)."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
