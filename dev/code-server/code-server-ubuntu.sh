...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║         Code-server Auto-Installer               ║"
...
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^code-server$' || true)
if [ -n "$EXISTING" ]; then
    warn "Removing existing container: code-server"
    docker rm -f code-server 2>/dev/null || true
fi
docker network prune -f &>/dev/null || true

section "Step 5: Preparing Directory"
CS_DIR="/root/docker/code-server"
if [ -d "$CS_DIR" ]; then
...
CS_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 20)
info "Password : $CS_PASS"
...
section "Step 7: Starting Code-server"
MAX_RETRIES=3
...
sleep 8
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^code-server$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs code-server"
else
...
if [ "$HEALTH_OK" -eq 0 ]; then
    warn "Code-server may still be starting. Check: docker logs code-server"
fi

section "Step 10: Opening Firewall Port 8094"
if command -v ufw &> /dev/null; then
    ufw allow 8094/tcp
    info "UFW: port 8094/tcp opened."
else
...
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║                                                      ║"
echo "  ║  🌐  Open Code-server in your browser:           ║"
echo "  ║      👉  http://$SERVER_IP:8094"
echo "  ║                                                      ║"
echo "  ║  🔑  Password : $CS_PASS"
echo "  ║                                                      ║"
echo "  ║  📁  Workspace : /root (host) mounted in IDE       ║"
echo "  ║                                                      ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️            ║"
echo "  ║       Made by: Mohammed Ali Elshikh                 ║"
echo "  ║       prismatechwork.com                            ║"
