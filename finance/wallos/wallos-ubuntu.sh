...
SERVICE_DIR="/root/docker/wallos"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(wallos)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/logos"
...
section "Step 7: Starting Wallos"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^wallos$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs wallos"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Wallos to be ready on port 8156..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8156 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8156 is responding (HTTP $STATUS) — Wallos is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8156 2>/dev/null; then
        warn "Port 8156 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8156 is NOT responding yet."
        docker logs --tail 20 wallos 2>&1 || true
    fi
    warn "Check logs: docker logs wallos"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  💳  Wallos (subscription tracker):  http://$SERVER_IP:8156"
