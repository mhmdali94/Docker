...
SERVICE_DIR="/root/docker/cassandra"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(cassandra)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Cassandra"
warn "Cassandra takes 1-2 minutes to initialize."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^cassandra$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs cassandra"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Cassandra on TCP port 9042..."
HEALTH_OK=0
for i in $(seq 1 36); do
    if nc -z 127.0.0.1 9042 2>/dev/null; then
        info "Port 9042 is open — Cassandra is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 9042 not responding yet. Check: docker logs cassandra"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  💠  Cassandra CQL:  $SERVER_IP:9042"
echo "  ║  🔧  cqlsh: docker exec -it cassandra cqlsh"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
