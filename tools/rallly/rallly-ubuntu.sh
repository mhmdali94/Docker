...
SERVICE_DIR="/root/docker/rallly"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(rallly|rallly-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  rallly:
    ...
    ports:
      - "3012:3000"
    environment:
      DATABASE_URL: postgres://rallly:$DB_PASS@rallly-db:5432/rallly
      SECRET_PASSWORD: $SECRET_PASSWORD
      NEXT_PUBLIC_BASE_URL: http://$SERVER_IP:3012
      ALLOWED_EMAILS: "*"
    networks:
      - rallly-net
...
section "Step 7: Starting Rallly"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^rallly$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs rallly"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Rallly to be ready on port 3012..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3012 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3012 is responding (HTTP $STATUS) — Rallly is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3012 2>/dev/null; then
        warn "Port 3012 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3012 is NOT responding yet."
        docker logs --tail 20 rallly 2>&1 || true
    fi
    warn "Check logs: docker logs rallly"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🗳️   Rallly (group scheduling):  http://$SERVER_IP:3012"
echo "  ║  ⚠️   Configure SMTP env vars to enable email login links."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
