...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Restreamer Auto-Installer"
...
SERVICE_DIR="/root/docker/restreamer"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(restreamer)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Restreamer"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^restreamer$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs restreamer"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Restreamer to be ready on port 8282..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8282 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8282 is responding (HTTP $STATUS) — Restreamer is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8282 2>/dev/null; then
        warn "Port 8282 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8282 is NOT responding yet."
        docker logs --tail 20 restreamer 2>&1 || true
    fi
    warn "Check logs: docker logs restreamer"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🎬  Restreamer:  http://$SERVER_IP:8282"
echo "  ║  🔑  Login: admin / $ADMIN_PASS"
echo "  ║  🎥  Ingest RTMP: rtmp://$SERVER_IP:1936"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
