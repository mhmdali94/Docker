...
SERVICE_DIR="/root/docker/questdb"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(questdb)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting QuestDB"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^questdb$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs questdb"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for QuestDB to be ready on port 9008..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:9008 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 9008 is responding (HTTP $STATUS) — QuestDB is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 9008 2>/dev/null; then
        warn "Port 9008 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 9008 is NOT responding yet."
        docker logs --tail 20 questdb 2>&1 || true
    fi
    warn "Check logs: docker logs questdb"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⏱️   QuestDB console:  http://$SERVER_IP:9008"
echo "  ║  🔌  PGWire: $SERVER_IP:8812 (admin/quest) | ILP ingest: $SERVER_IP:9011"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
