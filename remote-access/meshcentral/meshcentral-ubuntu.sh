...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   MeshCentral Auto-Installer"
...
SERVICE_DIR="/root/docker/meshcentral"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(meshcentral)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/data" "$SERVICE_DIR/files" "$SERVICE_DIR/backups"
...
section "Step 7: Starting MeshCentral"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^meshcentral$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs meshcentral"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for MeshCentral to be ready on port 8446..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -k -o /dev/null -w "%{http_code}" --max-time 5 https://127.0.0.1:8446 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8446 is responding (HTTP $STATUS) — MeshCentral is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8446 2>/dev/null; then
        warn "Port 8446 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8446 is NOT responding yet."
        docker logs --tail 20 meshcentral 2>&1 || true
    fi
    warn "Check logs: docker logs meshcentral"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🖥️   MeshCentral (remote management):  https://$SERVER_IP:8446"
