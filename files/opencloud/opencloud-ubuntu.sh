...
SERVICE_DIR="/root/docker/opencloud"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(opencloud)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/config" "$SERVICE_DIR/data"
...
chown -R 1000:1000 "$SERVICE_DIR/config" "$SERVICE_DIR/data"
...
section "Step 7: Starting OpenCloud"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^opencloud$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs opencloud"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for OpenCloud to be ready on port 9201..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -k -o /dev/null -w "%{http_code}" --max-time 5 https://127.0.0.1:9201 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 9201 is responding (HTTP $STATUS) — OpenCloud is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 9201 2>/dev/null; then
        warn "Port 9201 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 9201 is NOT responding yet."
        docker logs --tail 20 opencloud 2>&1 || true
    fi
    warn "Check logs: docker logs opencloud"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ☁️   OpenCloud (file sync & share):  https://$SERVER_IP:9201"
