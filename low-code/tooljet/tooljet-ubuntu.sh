...
for cname in tooljet tooljet-db; do
...
services:
  ...
  tooljet:
    ...
    environment:
      ...
      TOOLJET_HOST: http://${SERVER_IP}:3000
      NODE_ENV: production
    depends_on:
      - tooljet-db
    command: ["./wait-for-it.sh", "tooljet-db:5432", "--", "node", "dist/tooljet.js"]
EOF
info "docker-compose.yml created."

section "Step 7: Starting ToolJet"
MAX_RETRIES=3
...
info "Waiting for ToolJet on port 3000..."
HEALTH_OK=0
...
[ "$HEALTH_OK" -eq 0 ] && warn "Not responding. Check: docker logs tooljet"
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🛠️  ToolJet (Low-Code Platform):                  ║"
