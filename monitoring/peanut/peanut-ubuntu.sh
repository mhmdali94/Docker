...
SERVICE_DIR="/root/docker/peanut"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(peanut)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting PeaNUT"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^peanut$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs peanut"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for PeaNUT to be ready on port 8314..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8314 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8314 is responding (HTTP $STATUS) — PeaNUT is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8314 2>/dev/null; then
        warn "Port 8314 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8314 is NOT responding yet."
        docker logs --tail 20 peanut 2>&1 || true
    fi
    warn "Check logs: docker logs peanut"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔋  PeaNUT (UPS dashboard):  http://$SERVER_IP:8314"
echo "  ║  🔧  Add your NUT server (e.g. host.docker.internal:3493) in the settings."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
