...
section "Step 7: Starting Odoo 18"
MAX_RETRIES=3
...
sleep 15
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^odoo18$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs odoo18"
else
...
if [ "$HEALTH_OK" -eq 0 ]; then
    warn "Odoo may still be loading. Check: docker logs odoo18"
fi

section "Step 10: Opening Firewall Port 8018"
if command -v ufw &> /dev/null; then
    ufw allow 8018/tcp
    info "UFW: port 8018/tcp opened."
else
...
echo "  ║                                                      ║"
echo "  ║  🌐  Odoo 18 URL:                                  ║"
echo "  ║      http://$SERVER_IP:8018"
echo "  ║                                                      ║"
echo "  ║  🗄️  Database Manager:                             ║"
echo "  ║  http://$SERVER_IP:8018/web/database/manager"
echo "  ║                                                      ║"
