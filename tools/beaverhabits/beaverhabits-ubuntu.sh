#!/bin/bash
# ============================================================
#   Beaver Habit Tracker Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Beaver Habit Tracker Auto-Installer"
...
SERVICE_DIR="/root/docker/beaverhabits"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(beaverhabits)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Beaver Habit Tracker"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^beaverhabits$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs beaverhabits"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Beaver Habit Tracker to be ready on port 8220..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8220 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8220 is responding (HTTP $STATUS) — Beaver Habit Tracker is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8220 2>/dev/null; then
        warn "Port 8220 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8220 is NOT responding yet."
        docker logs --tail 20 beaverhabits 2>&1 || true
    fi
    warn "Check logs: docker logs beaverhabits"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🦫  Beaver Habit Tracker:  http://$SERVER_IP:8220"
