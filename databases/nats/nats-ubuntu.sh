...
SERVICE_DIR="/root/docker/nats"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(nats)$' || true)
if [ -n "$EXISTING" ]; then
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^nats$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs nats"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for NATS to be ready on port 8222..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8222 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8222 is responding (HTTP $STATUS) — NATS is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8222 2>/dev/null; then
        warn "Port 8222 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8222 is NOT responding yet."
        docker logs --tail 20 nats 2>&1 || true
    fi
    warn "Check logs: docker logs nats"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📡  NATS (with JetStream):  nats://$SERVER_IP:4222"
echo "  ║  🖥️   Monitoring: http://$SERVER_IP:8222"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
