...
SERVICE_DIR="/root/docker/joomla"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(joomla|joomla-db)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/html"
...
section "Step 7: Starting Joomla"
warn "First start auto-installs Joomla — allow 2-3 minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^joomla$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs joomla"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Joomla to be ready on port 8147..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8147 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8147 is responding (HTTP $STATUS) — Joomla is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8147 2>/dev/null; then
        warn "Port 8147 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8147 is NOT responding yet."
        docker logs --tail 20 joomla 2>&1 || true
    fi
    warn "Check logs: docker logs joomla"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🌐  Joomla:  http://$SERVER_IP:8147"
echo "  ║  ⚙️   Admin: http://$SERVER_IP:8147/administrator"
echo "  ║  🔑  Login: joomla / $ADMIN_PASS"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
