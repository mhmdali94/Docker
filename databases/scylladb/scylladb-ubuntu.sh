...
SERVICE_DIR="/root/docker/scylladb"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(scylladb)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting ScyllaDB"
warn "ScyllaDB takes a minute to initialize."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^scylladb$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs scylladb"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for ScyllaDB on TCP port 9043..."
HEALTH_OK=0
for i in $(seq 1 36); do
    if nc -z 127.0.0.1 9043 2>/dev/null; then
        info "Port 9043 is open — ScyllaDB is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 9043 not responding yet. Check: docker logs scylladb"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⚡  ScyllaDB CQL:  $SERVER_IP:9043"
echo "  ║  🔧  cqlsh: docker exec -it scylladb cqlsh"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
