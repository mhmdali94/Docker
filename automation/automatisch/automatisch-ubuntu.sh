...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Automatisch Auto-Installer"
...
SERVICE_DIR="/root/docker/automatisch"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(automatisch|automatisch-worker|automatisch-db|automatisch-redis)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/storage"
...
section "Step 7: Starting Automatisch"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^automatisch$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs automatisch"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Automatisch to be ready on port 3023..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3023 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3023 is responding (HTTP $STATUS) — Automatisch is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3023 2>/dev/null; then
        warn "Port 3023 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3023 is NOT responding yet."
        docker logs --tail 20 automatisch 2>&1 || true
    fi
    warn "Check logs: docker logs automatisch"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⚡  Automatisch (open-source Zapier):  http://$SERVER_IP:3023"
echo "  ║  🔑  Default login: user@automatisch.io / sample  (change it!)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
