...
SERVICE_DIR="/root/docker/xibo"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(xibo-cms|xibo-db|xibo-xmr)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/library"
...
section "Step 7: Starting Xibo"
warn "Xibo initializes its database on first start — allow a few minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^xibo-cms$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs xibo-cms"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Xibo to be ready on port 8320..."
HEALTH_OK=0
for i in $(seq 1 48); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8320 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8320 is responding (HTTP $STATUS) — Xibo is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8320 2>/dev/null; then
        warn "Port 8320 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8320 is NOT responding yet."
        docker logs --tail 20 xibo-cms 2>&1 || true
    fi
    warn "Check logs: docker logs xibo-cms"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📺  Xibo (digital signage CMS):  http://$SERVER_IP:8320"
echo "  ║  🔑  Default login: xibo_admin / password  (change it!)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
