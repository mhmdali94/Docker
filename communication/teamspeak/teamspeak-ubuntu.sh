#!/bin/bash
# ============================================================
#   TeamSpeak Server Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   TeamSpeak Server Auto-Installer"
...
SERVICE_DIR="/root/docker/teamspeak"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(teamspeak)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting TeamSpeak Server"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^teamspeak$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs teamspeak"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for TeamSpeak Server on TCP port 30033..."
HEALTH_OK=0
for i in $(seq 1 24); do
    if nc -z 127.0.0.1 30033 2>/dev/null; then
        info "Port 30033 is open — TeamSpeak Server is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 30033 not responding yet. Check: docker logs teamspeak"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🎙️   TeamSpeak Server:  $SERVER_IP:9987"
echo "  ║  🔑  ServerAdmin token is printed on first start:"
echo "  ║      docker logs teamspeak 2>&1 | grep -i token"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
