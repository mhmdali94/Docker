...
SERVICE_DIR="/root/docker/wikijs"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(wikijs|wikijs-db)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Wiki.js"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^wikijs$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs wikijs"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Wiki.js to be ready on port 3009..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3009 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3009 is responding (HTTP $STATUS) — Wiki.js is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3009 2>/dev/null; then
        warn "Port 3009 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3009 is NOT responding yet."
        docker logs --tail 20 wikijs 2>&1 || true
    fi
    warn "Check logs: docker logs wikijs"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🌐  Wiki.js:  http://$SERVER_IP:3009"
echo "  ║  🔑  Create the admin account in the setup wizard on first visit."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
