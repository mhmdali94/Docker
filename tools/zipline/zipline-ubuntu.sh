...
SERVICE_DIR="/root/docker/zipline"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(zipline|zipline-db)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/uploads" "$SERVICE_DIR/public"
...
services:
  ...
  zipline:
    ...
    environment:
      ...
      DATABASE_URL: postgresql://zipline:$DB_PASS@zipline-db:5432/zipline
    volumes:
...
section "Step 7: Starting Zipline"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^zipline$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs zipline"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Zipline to be ready on port 3053..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3053 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3053 is responding (HTTP $STATUS) — Zipline is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3053 2>/dev/null; then
        warn "Port 3053 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3053 is NOT responding yet."
        docker logs --tail 20 zipline 2>&1 || true
    fi
    warn "Check logs: docker logs zipline"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📸  Zipline (ShareX/screenshot server):  http://$SERVER_IP:3053"
