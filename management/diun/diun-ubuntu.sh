...
SERVICE_DIR="/root/docker/diun"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(diun)$' || true)
if [ -n "$EXISTING" ]; then
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^diun$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs diun"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Diun to finish starting (log check)..."
HEALTH_OK=0
for i in $(seq 1 6); do
    if docker logs diun 2>&1 | grep -qiE 'diun'; then
        info "Diun is ready. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Still starting. Monitor: docker logs -f diun"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔔  Diun is watching all running containers for image updates (every 6h)."
echo "  ║  🔧  Add a notifier (Telegram, Gotify, email...) via DIUN_NOTIF_* env vars in docker-compose.yml."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
