...
SERVICE_DIR="/root/docker/ferretdb"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(ferretdb|ferretdb-pg)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  ferretdb:
    ...
    ports:
      - "27018:27017"
    environment:
      FERRETDB_POSTGRESQL_URL: postgres://ferret:$DB_PASS@ferretdb-pg:5432/postgres
    networks:
      - ferretdb-net
...
section "Step 7: Starting FerretDB"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^ferretdb$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs ferretdb"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for FerretDB on TCP port 27018..."
HEALTH_OK=0
for i in $(seq 1 36); do
    if nc -z 127.0.0.1 27018 2>/dev/null; then
        info "Port 27018 is open — FerretDB is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 27018 not responding yet. Check: docker logs ferretdb"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🦦  FerretDB (MongoDB-compatible on Postgres):  $SERVER_IP:27018"
echo "  ║  🔑  mongodb://ferret:$DB_PASS@$SERVER_IP:27018/"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
