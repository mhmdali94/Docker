...
if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qE '^netdata$'; then
    warn "Removing existing container: netdata"
    docker rm -f netdata 2>/dev/null || true
fi
docker network prune -f &>/dev/null || true

section "Step 5: Preparing Directory"
ND_DIR="/root/docker/netdata"
if [ -d "$ND_DIR" ]; then
...
for i in $(seq 1 12); do
    if curl -s --max-time 5 http://127.0.0.1:19999/api/v1/info &>/dev/null; then
...
[ "$HEALTH_OK" -eq 0 ] && warn "Netdata may still be initializing. Check: docker logs netdata"

section "Step 10: Opening Firewall Port 19999"
if command -v ufw &> /dev/null; then
    ufw allow 19999/tcp
    info "UFW: port 19999/tcp opened."
else
...
echo "  ║                                                      ║"
echo "  ║  📊  Real-time server metrics — no login needed.   ║"
echo "  ║                                                      ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️            ║"
echo "  ║       Made by: Mohammed Ali Elshikh                 ║"
echo "  ║       prismatechwork.com                            ║"
