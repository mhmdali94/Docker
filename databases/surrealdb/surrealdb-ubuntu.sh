...
SERVICE_DIR="/root/docker/surrealdb"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(surrealdb)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  surrealdb:
    ...
    user: root
    command: start --user root --pass $ROOT_PASS rocksdb:/data/database.db
    ports:
      - "8149:8000"
    volumes:
      - ./data:/data
...
section "Step 7: Starting SurrealDB"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^surrealdb$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs surrealdb"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for SurrealDB to be ready on port 8149..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8149/health 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8149 is responding (HTTP $STATUS) — SurrealDB is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8149 2>/dev/null; then
        warn "Port 8149 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8149 is NOT responding yet."
        docker logs --tail 20 surrealdb 2>&1 || true
    fi
    warn "Check logs: docker logs surrealdb"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🌀  SurrealDB:  http://$SERVER_IP:8149"
