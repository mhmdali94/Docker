#!/bin/bash
# ============================================================
#   addy.io (AnonAddy) Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   addy.io (AnonAddy) Auto-Installer"
...
SERVICE_DIR="/root/docker/addy"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(addy|addy-db|addy-redis)$' || true)
if [ -n "$EXISTING" ]; then
...
read -rp "  Mail alias domain (e.g. example.com): " ADDY_DOMAIN
[ -z "$ADDY_DOMAIN" ] && error "A domain is required."
DB_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 24)
APP_KEY="base64:$(openssl rand -base64 32)"
SECRET=$(openssl rand -hex 32)
info "Domain: $ADDY_DOMAIN"
...
services:
  ...
  addy:
    ...
    environment:
      APP_KEY: $APP_KEY
      ANONADDY_DOMAIN: $ADDY_DOMAIN
      ANONADDY_SECRET: $SECRET
      DB_HOST: addy-db
      DB_DATABASE: anonaddy
      DB_USERNAME: anonaddy
      DB_PASSWORD: $DB_PASS
      REDIS_HOST: addy-redis
...
section "Step 7: Starting addy.io (AnonAddy)"
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^addy$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs addy"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for addy.io (AnonAddy) to be ready on port 8190..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8190 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8190 is responding (HTTP $STATUS) — addy.io (AnonAddy) is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8190 2>/dev/null; then
        warn "Port 8190 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8190 is NOT responding yet."
        docker logs --tail 20 addy 2>&1 || true
    fi
    warn "Check logs: docker logs addy"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📮  addy.io (email aliases):  http://$SERVER_IP:8190"
echo "  ║  📋  DNS: point MX for $ADDY_DOMAIN at this server + SPF/DKIM/DMARC for deliverability."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
