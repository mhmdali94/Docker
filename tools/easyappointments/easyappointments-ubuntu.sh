#!/bin/bash
# ============================================================
#   Easy!Appointments Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Easy!Appointments Auto-Installer"
...
SERVICE_DIR="/root/docker/easyappointments"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(easyappointments|easyappointments-db)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Easy!Appointments"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^easyappointments$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs easyappointments"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Easy!Appointments to be ready on port 8250..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8250 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8250 is responding (HTTP $STATUS) — Easy!Appointments is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8250 2>/dev/null; then
        warn "Port 8250 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8250 is NOT responding yet."
        docker logs --tail 20 easyappointments 2>&1 || true
    fi
    warn "Check logs: docker logs easyappointments"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📅  Easy!Appointments:  http://$SERVER_IP:8250"
echo "  ║  🔧  Complete the installation wizard on first visit."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
