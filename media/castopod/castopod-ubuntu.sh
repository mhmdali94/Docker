...
SERVICE_DIR="/root/docker/castopod"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(castopod|castopod-db|castopod-redis)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Castopod"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^castopod$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs castopod"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Castopod to be ready on port 8305..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8305 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8305 is responding (HTTP $STATUS) — Castopod is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8305 2>/dev/null; then
        warn "Port 8305 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8305 is NOT responding yet."
        docker logs --tail 20 castopod 2>&1 || true
    fi
    warn "Check logs: docker logs castopod"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🎙️   Castopod (podcast hosting):  http://$SERVER_IP:8305"
echo "  ║  🔧  Create the superadmin at: http://$SERVER_IP:8305/cp-install"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
