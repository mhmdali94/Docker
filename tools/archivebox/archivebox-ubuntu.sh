...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   ArchiveBox Auto-Installer"
...
SERVICE_DIR="/root/docker/archivebox"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(archivebox)$' || true)
if [ -n "$EXISTING" ]; then
...
info "Admin user: admin / $ADMIN_PASS"
...
services:
  archivebox:
    ...
    environment:
      ...
      ALLOWED_HOSTS: "*"
      CSRF_TRUSTED_ORIGINS: http://$SERVER_IP:8112
    volumes:
      - ./data:/data
EOF
info "docker-compose.yml created."

section "Step 7: Starting ArchiveBox"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^archivebox$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs archivebox"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for ArchiveBox to be ready on port 8112..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8112 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8112 is responding (HTTP $STATUS) — ArchiveBox is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8112 2>/dev/null; then
        warn "Port 8112 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8112 is NOT responding yet."
        docker logs --tail 20 archivebox 2>&1 || true
    fi
    warn "Check logs: docker logs archivebox"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🗃️   ArchiveBox:  http://$SERVER_IP:8112"
