...
SERVICE_DIR="/root/docker/convertx"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(convertx)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting ConvertX"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^convertx$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs convertx"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for ConvertX to be ready on port 3042..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3042 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3042 is responding (HTTP $STATUS) — ConvertX is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3042 2>/dev/null; then
        warn "Port 3042 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3042 is NOT responding yet."
        docker logs --tail 20 convertx 2>&1 || true
    fi
    warn "Check logs: docker logs convertx"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔄  ConvertX (file converter, 1000+ formats):  http://$SERVER_IP:3042"
