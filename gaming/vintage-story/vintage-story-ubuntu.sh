#!/bin/bash
# ============================================================
#   Vintage Story Server Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Vintage Story Server Auto-Installer"
...
SERVICE_DIR="/root/docker/vintage-story"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(vintage-story)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Vintage Story Server"
warn "Downloading Vintage Story server — first start takes a few minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^vintage-story$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs vintage-story"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Vintage Story Server on TCP port 42420..."
HEALTH_OK=0
for i in $(seq 1 36); do
    if nc -z 127.0.0.1 42420 2>/dev/null; then
        info "Port 42420 is open — Vintage Story Server is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 42420 not responding yet. Check: docker logs vintage-story"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🏹  Vintage Story Server:  $SERVER_IP:42420"
echo "  ║  🔧  Config: $SERVICE_DIR/data/serverconfig.json (set password, name, whitelist)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
