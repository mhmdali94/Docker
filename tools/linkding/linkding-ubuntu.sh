...
SERVICE_DIR="/root/docker/linkding"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(linkding)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting linkding"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^linkding$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs linkding"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for linkding to be ready on port 9095..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:9095 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 9095 is responding (HTTP $STATUS) — linkding is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 9095 2>/dev/null; then
        warn "Port 9095 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 9095 is NOT responding yet."
        docker logs --tail 20 linkding 2>&1 || true
    fi
    warn "Check logs: docker logs linkding"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔖  linkding (bookmarks):  http://$SERVER_IP:9095"
