...
SERVICE_DIR="/root/docker/tryton"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(tryton|tryton-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  tryton:
    ...
    command:
      ...
      - -c
      - |
        (until echo > /dev/tcp/tryton-db/5432; do sleep 0.5; done) 2>/dev/null
        echo "$ADMIN_PASS" > /tmp/.passwd
        TRYTONPASSFILE=/tmp/.passwd /entrypoint.sh trytond-admin -d tryton --all --email admin@tryton.local -vv
...
section "Step 7: Starting Tryton"
warn "First start initializes the Tryton database — allow a few minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^tryton$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs tryton"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Tryton to be ready on port 8153..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8153 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8153 is responding (HTTP $STATUS) — Tryton is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8153 2>/dev/null; then
        warn "Port 8153 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8153 is NOT responding yet."
        docker logs --tail 20 tryton 2>&1 || true
    fi
    warn "Check logs: docker logs tryton"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🏢  Tryton ERP:  http://$SERVER_IP:8153"
echo "  ║  🔑  Database: tryton | Login: admin / $ADMIN_PASS"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
