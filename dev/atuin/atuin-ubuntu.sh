...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Atuin Server Auto-Installer"
...
SERVICE_DIR="/root/docker/atuin"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(atuin|atuin-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  atuin:
    ...
    environment:
      ...
      ATUIN_DB_URI: postgres://atuin:$DB_PASS@atuin-db/atuin
    networks:
      - atuin-net
...
section "Step 7: Starting Atuin Server"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^atuin$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs atuin"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Atuin Server to be ready on port 8888..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8888 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8888 is responding (HTTP $STATUS) — Atuin Server is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8888 2>/dev/null; then
        warn "Port 8888 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8888 is NOT responding yet."
        docker logs --tail 20 atuin 2>&1 || true
    fi
    warn "Check logs: docker logs atuin"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⌨️   Atuin sync server:  http://$SERVER_IP:8888"
echo "  ║  🔧  On clients: atuin login --host http://$SERVER_IP:8888 (registration open)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
