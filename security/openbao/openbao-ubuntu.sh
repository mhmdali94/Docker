...
SERVICE_DIR="/root/docker/openbao"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(openbao)$' || true)
if [ -n "$EXISTING" ]; then
...
ROOT_TOKEN=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 24)
...
section "Step 7: Starting OpenBao"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^openbao$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs openbao"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for OpenBao to be ready on port 8245..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8245/v1/sys/health 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8245 is responding (HTTP $STATUS) — OpenBao is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8245 2>/dev/null; then
        warn "Port 8245 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8245 is NOT responding yet."
        docker logs --tail 20 openbao 2>&1 || true
    fi
    warn "Check logs: docker logs openbao"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔐  OpenBao (Vault fork):  http://$SERVER_IP:8245"
echo "  ║  🔑  Root token: $ROOT_TOKEN"
echo "  ║  ⚠️   Dev mode (in-memory) — use a real storage config for anything persistent."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
