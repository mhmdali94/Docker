...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Mumble Server Auto-Installer"
...
SERVICE_DIR="/root/docker/mumble"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(mumble)$' || true)
if [ -n "$EXISTING" ]; then
...
SU_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 16)
SERVER_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 12)
info "SuperUser password: $SU_PASS | Server password: $SERVER_PASS"
...
services:
  mumble:
    ...
    environment:
      ...
      MUMBLE_CONFIG_SERVER_PASSWORD: $SERVER_PASS
...
section "Step 7: Starting Mumble Server"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^mumble$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs mumble"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Mumble Server on TCP port 64738..."
HEALTH_OK=0
for i in $(seq 1 24); do
    if nc -z 127.0.0.1 64738 2>/dev/null; then
        info "Port 64738 is open — Mumble Server is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 64738 not responding yet. Check: docker logs mumble"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🎙️   Mumble Server:  $SERVER_IP:64738"
echo "  ║  🔑  Server password: $SERVER_PASS"
echo "  ║  🔑  SuperUser password: $SU_PASS"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
