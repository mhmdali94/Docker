...
SERVICE_DIR="/root/docker/grocy"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(grocy)$' || true)
if [ -n "$EXISTING" ]; then
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^grocy$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs grocy"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Grocy to be ready on port 9283..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:9283 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 9283 is responding (HTTP $STATUS) — Grocy is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 9283 2>/dev/null; then
        warn "Port 9283 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 9283 is NOT responding yet."
        docker logs --tail 20 grocy 2>&1 || true
    fi
    warn "Check logs: docker logs grocy"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🛒  Grocy (household ERP):  http://$SERVER_IP:9283"
