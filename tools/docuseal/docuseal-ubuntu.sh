...
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E "^docuseal$" || true)
[ -n "$EXISTING" ] && warn "Removing existing container..." && docker rm -f docuseal 2>/dev/null || true

section "Step 5: Preparing Directory"
APP_DIR="/root/docker/docuseal"
...
section "Step 7: Starting DocuSeal"
if docker compose version &> /dev/null; then
