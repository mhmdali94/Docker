...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Horilla HRMS Auto-Installer"
...
SERVICE_DIR="/root/docker/horilla"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(horilla|horilla-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  horilla:
    ...
    environment:
      ...
      ALLOWED_HOSTS: "*"
      CSRF_TRUSTED_ORIGINS: http://$SERVER_IP:8157
...
section "Step 7: Starting Horilla HRMS"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^horilla$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs horilla"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Horilla HRMS to be ready on port 8157..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8157 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8157 is responding (HTTP $STATUS) — Horilla HRMS is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8157 2>/dev/null; then
        warn "Port 8157 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8157 is NOT responding yet."
        docker logs --tail 20 horilla 2>&1 || true
    fi
    warn "Check logs: docker logs horilla"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  👥  Horilla HRMS:  http://$SERVER_IP:8157"
echo "  ║  🔧  First visit: open /initialize-database to create the admin account."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
