...
for cname in wordpress wordpress-db; do
...
section "Step 7: Starting WordPress"
MAX_RETRIES=3
...
sleep 8
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^wordpress$' || true)
[ -z "$RUNNING" ] && warn "Container may not have started. Check: docker logs wordpress" || info "Container running: $RUNNING"

section "Step 9: Health Check"
info "Waiting for WordPress on port 8090..."
HEALTH_OK=0
for i in $(seq 1 12); do
    if curl -s --max-time 3 http://127.0.0.1:8090 &>/dev/null; then
        info "Port 8090 is responding. ✅"; HEALTH_OK=1; break
    fi
    echo -n "  Attempt $i/12 — waiting 5s..."; sleep 5; echo " retrying"
done
[ "$HEALTH_OK" -eq 0 ] && warn "Not responding. Check: docker logs wordpress"

section "Step 10: Opening Firewall"
if command -v ufw &>/dev/null; then
    ufw allow 8090/tcp; info "UFW: port 8090 opened."
else
