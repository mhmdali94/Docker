...
SERVICE_DIR="/root/docker/litellm"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(litellm)$' || true)
if [ -n "$EXISTING" ]; then
...
MASTER_KEY="sk-$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 32)"
...
section "Step 7: Starting LiteLLM"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^litellm$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs litellm"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for LiteLLM to be ready on port 4001..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:4001/health/liveliness 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 4001 is responding (HTTP $STATUS) — LiteLLM is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 4001 2>/dev/null; then
        warn "Port 4001 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 4001 is NOT responding yet."
        docker logs --tail 20 litellm 2>&1 || true
    fi
    warn "Check logs: docker logs litellm"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🧠  LiteLLM proxy:  http://$SERVER_IP:4001"
echo "  ║  🔑  Master key: $MASTER_KEY"
echo "  ║  🔗  OpenAI-compatible endpoint: http://$SERVER_IP:4001/v1"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
