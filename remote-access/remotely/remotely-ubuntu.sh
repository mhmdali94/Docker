...
#
# ============================================================
#   Remotely Remote Desktop Support Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║    Remotely Remote Desktop Support Installer     ║"
...
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E 'remotely' || true)
if [ -n "$EXISTING" ]; then
    warn "Removing existing containers..."
    echo "$EXISTING" | xargs docker rm -f 2>/dev/null || true
else
    info "No existing Remotely containers found."
fi
docker network prune -f &>/dev/null || true

section "Step 5: Preparing Directory"
REMOTELY_DIR="/root/docker/remotely"
if [ -d "$REMOTELY_DIR" ]; then
    warn "Removing old directory $REMOTELY_DIR..."
    rm -rf "$REMOTELY_DIR"
fi
mkdir -p "$REMOTELY_DIR"
cd "$REMOTELY_DIR" || error "Cannot navigate to $REMOTELY_DIR"
info "Directory ready: $REMOTELY_DIR"

section "Step 6: Generating docker-compose.yml"
cat > "$REMOTELY_DIR/docker-compose.yml" <<EOF
services:
  remotely:
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 5000 2>/dev/null; then
        ...
        warn "Check logs: docker logs remotely"
    else
        warn "Port 5000 is NOT responding after 60s."
        warn "Check logs: docker logs remotely"
        docker logs --tail 20 remotely 2>&1 || true
    fi
fi
