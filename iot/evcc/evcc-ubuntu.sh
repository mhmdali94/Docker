...
SERVICE_DIR="/root/docker/evcc"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(evcc)$' || true)
if [ -n "$EXISTING" ]; then
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^evcc$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs evcc"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for evcc to be ready on port 7070..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:7070 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 7070 is responding (HTTP $STATUS) — evcc is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 7070 2>/dev/null; then
        warn "Port 7070 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 7070 is NOT responding yet."
        docker logs --tail 20 evcc 2>&1 || true
    fi
    warn "Check logs: docker logs evcc"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⚡  evcc (EV charging, demo mode):  http://$SERVER_IP:7070"
echo "  ║  🔧  Replace --demo with a real evcc.yaml for your wallbox/PV setup."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
