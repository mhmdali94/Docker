...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   LocalStack Auto-Installer"
...
SERVICE_DIR="/root/docker/localstack"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(localstack)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting LocalStack"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^localstack$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs localstack"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for LocalStack to be ready on port 4566..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:4566/_localstack/health 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 4566 is responding (HTTP $STATUS) — LocalStack is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 4566 2>/dev/null; then
        warn "Port 4566 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 4566 is NOT responding yet."
        docker logs --tail 20 localstack 2>&1 || true
    fi
    warn "Check logs: docker logs localstack"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ☁️   LocalStack (AWS emulator):  http://$SERVER_IP:4566"
echo "  ║  🔧  aws --endpoint-url=http://$SERVER_IP:4566 s3 mb s3://test"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
