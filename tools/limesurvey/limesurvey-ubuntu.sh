...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   LimeSurvey Auto-Installer"
...
SERVICE_DIR="/root/docker/limesurvey"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(limesurvey|limesurvey-db)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/surveys"
...
services:
  ...
  limesurvey:
    ...
    environment:
      ...
      PUBLIC_URL: http://$SERVER_IP:8306
    volumes:
...
section "Step 7: Starting LimeSurvey"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^limesurvey$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs limesurvey"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for LimeSurvey to be ready on port 8306..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8306 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8306 is responding (HTTP $STATUS) — LimeSurvey is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8306 2>/dev/null; then
        warn "Port 8306 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8306 is NOT responding yet."
        docker logs --tail 20 limesurvey 2>&1 || true
    fi
    warn "Check logs: docker logs limesurvey"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📋  LimeSurvey:  http://$SERVER_IP:8306"
echo "  ║  ⚙️   Admin: http://$SERVER_IP:8306/index.php/admin  (admin / $ADMIN_PASS)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
