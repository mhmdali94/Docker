...
SERVICE_DIR="/root/docker/onedev"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(onedev)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting OneDev"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^onedev$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs onedev"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for OneDev to be ready on port 6610..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:6610 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 6610 is responding (HTTP $STATUS) — OneDev is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 6610 2>/dev/null; then
        warn "Port 6610 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 6610 is NOT responding yet."
        docker logs --tail 20 onedev 2>&1 || true
    fi
    warn "Check logs: docker logs onedev"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🛠️   OneDev (Git + CI/CD + Kanban):  http://$SERVER_IP:6610"
echo "  ║  🔧  Create the admin account in the setup wizard."
echo "  ║  🔐  Git SSH port: 6611"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
