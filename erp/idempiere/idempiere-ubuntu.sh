...
for C in idempiere idempiere-db; do
...
ID_DIR="/root/docker/idempiere"
if [ -d "$ID_DIR" ]; then
    warn "Removing old directory $ID_DIR..."
    rm -rf "$ID_DIR"
fi
mkdir -p "$ID_DIR"
cd "$ID_DIR" || error "Cannot navigate to $ID_DIR"
info "Directory ready: $ID_DIR"

section "Step 6: Writing docker-compose.yml"
DB_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 20)
info "DB Password : $DB_PASS"
warn "Default login after startup: GardenAdmin / GardenAdmin — change immediately."

cat > "$ID_DIR/docker-compose.yml" <<EOF
services:
  idempiere-db:
    image: postgres:13
  ...
  idempiere:
    ...
    volumes:
      - ./data:/opt/idempiere/data
EOF
info "docker-compose.yml created."

section "Step 7: Starting iDempiere"
MAX_RETRIES=3
...
sleep 20
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^idempiere$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs idempiere"
else
...
if [ "$HEALTH_OK" -eq 0 ]; then
    warn "iDempiere may still be starting. Check: docker logs idempiere"
fi

section "Step 10: Opening Firewall Port 8121"
if command -v ufw &> /dev/null; then
    ufw allow 8121/tcp
    info "UFW: port 8121/tcp opened."
else
...
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║                                                      ║"
echo "  ║  🌐  iDempiere Web UI:                             ║"
echo "  ║      http://$SERVER_IP:8121/webui/"
echo "  ║                                                      ║"
echo "  ║  🔑  Default Login (change immediately!):          ║"
echo "  ║      Username : GardenAdmin"
echo "  ║      Password : GardenAdmin"
echo "  ║                                                      ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️            ║"
echo "  ║       Made by: Mohammed Ali Elshikh                 ║"
echo "  ║       prismatechwork.com                            ║"
