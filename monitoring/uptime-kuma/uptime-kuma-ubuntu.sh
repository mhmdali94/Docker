...
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E 'uptime-kuma' || true)
if [ -n "$EXISTING" ]; then
    warn "Removing existing containers..."
    echo "$EXISTING" | xargs docker rm -f 2>/dev/null || true
else
    info "No existing Uptime Kuma containers found."
fi
docker network prune -f &>/dev/null || true

section "Step 5: Preparing Directory"
KUMA_DIR="/root/docker/uptime-kuma"
if [ -d "$KUMA_DIR" ]; then
    warn "Removing old directory $KUMA_DIR..."
    rm -rf "$KUMA_DIR"
fi
mkdir -p "$KUMA_DIR"
cd "$KUMA_DIR" || error "Cannot navigate to $KUMA_DIR"
info "Directory ready: $KUMA_DIR"
