...
SERVICE_DIR="/root/docker/manyfold"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(manyfold|manyfold-db|manyfold-redis)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/libraries"
...
section "Step 7: Starting Manyfold"
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^manyfold$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs manyfold"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Manyfold to be ready on port 3214..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3214 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3214 is responding (HTTP $STATUS) — Manyfold is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3214 2>/dev/null; then
        warn "Port 3214 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3214 is NOT responding yet."
        docker logs --tail 20 manyfold 2>&1 || true
    fi
    warn "Check logs: docker logs manyfold"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🗿  Manyfold (3D model library):  http://$SERVER_IP:3214"
echo "  ║  🔧  First registered user becomes admin. Put STL folders in $SERVICE_DIR/libraries"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
