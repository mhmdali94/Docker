...
SERVICE_DIR="/root/docker/judge0"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(judge0|judge0-workers|judge0-db|judge0-redis)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Judge0"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^judge0$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs judge0"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Judge0 to be ready on port 2358..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:2358/languages 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 2358 is responding (HTTP $STATUS) — Judge0 is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 2358 2>/dev/null; then
        warn "Port 2358 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 2358 is NOT responding yet."
        docker logs --tail 20 judge0 2>&1 || true
    fi
    warn "Check logs: docker logs judge0"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⚖️   Judge0 (code execution API):  http://$SERVER_IP:2358"
echo "  ║  🔗  Docs: http://$SERVER_IP:2358/docs — try GET /languages"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
