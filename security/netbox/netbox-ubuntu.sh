...
SERVICE_DIR="/root/docker/netbox"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(netbox|netbox-worker|netbox-db|netbox-redis)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  netbox:
    ...
    environment: &netbox-env
      ...
      REDIS_CACHE_PASSWORD: $REDIS_PASS
      REDIS_CACHE_DATABASE: 1
      SECRET_KEY: $SECRET_KEY
      SKIP_SUPERUSER: "false"
      SUPERUSER_NAME: admin
      SUPERUSER_EMAIL: admin@localhost.local
...
section "Step 7: Starting NetBox"
warn "First start runs migrations — allow several minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^netbox$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs netbox"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for NetBox to be ready on port 8162..."
HEALTH_OK=0
for i in $(seq 1 48); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8162 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8162 is responding (HTTP $STATUS) — NetBox is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8162 2>/dev/null; then
        warn "Port 8162 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8162 is NOT responding yet."
        docker logs --tail 20 netbox 2>&1 || true
    fi
    warn "Check logs: docker logs netbox"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🗺️   NetBox (IPAM/DCIM):  http://$SERVER_IP:8162"
