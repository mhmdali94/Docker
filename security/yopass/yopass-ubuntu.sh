...
SERVICE_DIR="/root/docker/yopass"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(yopass|yopass-memcached)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  yopass:
    ...
    depends_on:
      - yopass-memcached
    command: --database=memcached --memcached=yopass-memcached:11211
    ports:
      - "8173:1337"
    networks:
      - yopass-net
...
section "Step 7: Starting Yopass"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^yopass$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs yopass"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Yopass to be ready on port 8173..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8173 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8173 is responding (HTTP $STATUS) — Yopass is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8173 2>/dev/null; then
        warn "Port 8173 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8173 is NOT responding yet."
        docker logs --tail 20 yopass 2>&1 || true
    fi
    warn "Check logs: docker logs yopass"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🤫  Yopass (one-time secret sharing):  http://$SERVER_IP:8173"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
