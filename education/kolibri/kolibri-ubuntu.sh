...
SERVICE_DIR="/root/docker/kolibri"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(kolibri)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Kolibri"
warn "First start prepares the Kolibri database — allow a few minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^kolibri$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs kolibri"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Kolibri to be ready on port 8196..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8196 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8196 is responding (HTTP $STATUS) — Kolibri is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8196 2>/dev/null; then
        warn "Port 8196 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8196 is NOT responding yet."
        docker logs --tail 20 kolibri 2>&1 || true
    fi
    warn "Check logs: docker logs kolibri"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🎓  Kolibri (offline-first learning):  http://$SERVER_IP:8196"
echo "  ║  🔧  Complete the setup wizard, then import channels from Kolibri Studio."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
