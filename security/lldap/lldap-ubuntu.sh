...
SERVICE_DIR="/root/docker/lldap"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(lldap)$' || true)
if [ -n "$EXISTING" ]; then
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^lldap$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs lldap"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for LLDAP to be ready on port 17170..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:17170 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 17170 is responding (HTTP $STATUS) — LLDAP is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 17170 2>/dev/null; then
        warn "Port 17170 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 17170 is NOT responding yet."
        docker logs --tail 20 lldap 2>&1 || true
    fi
    warn "Check logs: docker logs lldap"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📒  LLDAP admin UI:  http://$SERVER_IP:17170"
echo "  ║  🔑  Login: admin / $ADMIN_PASS"
echo "  ║  🔗  LDAP: ldap://$SERVER_IP:3890  base DN: dc=example,dc=com"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
