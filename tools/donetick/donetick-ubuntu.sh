...
SERVICE_DIR="/root/docker/donetick"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(donetick)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/data" "$SERVICE_DIR/config"
...
section "Step 7: Starting Donetick"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^donetick$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs donetick"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Donetick to be ready on port 2021..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:2021 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 2021 is responding (HTTP $STATUS) — Donetick is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 2021 2>/dev/null; then
        warn "Port 2021 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 2021 is NOT responding yet."
        docker logs --tail 20 donetick 2>&1 || true
    fi
    warn "Check logs: docker logs donetick"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ✅  Donetick (chores & tasks for households):  http://$SERVER_IP:2021"
