...
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E 'portainer' || true)
if [ -n "$EXISTING" ]; then
    warn "Removing existing containers..."
    echo "$EXISTING" | xargs docker rm -f 2>/dev/null || true
else
    info "No existing Portainer containers found."
fi
docker network prune -f &>/dev/null || true

section "Step 5: Preparing Directory"
PORTAINER_DIR="/root/docker/portainer"
if [ -d "$PORTAINER_DIR" ]; then
    warn "Removing old directory $PORTAINER_DIR..."
    rm -rf "$PORTAINER_DIR"
fi
mkdir -p "$PORTAINER_DIR"
cd "$PORTAINER_DIR" || error "Cannot navigate to $PORTAINER_DIR"
info "Directory ready: $PORTAINER_DIR"
...
section "Step 7: Starting Portainer"
if docker compose version &> /dev/null; then
    docker compose up -d
else
    docker-compose up -d
...
echo "  ║      (you have 5 minutes before it locks)           ║"
echo "  ║                                                      ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️            ║"
echo "  ║       Made by: Mohammed Ali Elshikh | prismatechwork.com                   ║"
echo "  ╚══════════════════════════════════════════════════════╝"
