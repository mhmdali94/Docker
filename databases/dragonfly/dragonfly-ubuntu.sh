...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   DragonflyDB Auto-Installer"
...
SERVICE_DIR="/root/docker/dragonfly"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(dragonfly)$' || true)
if [ -n "$EXISTING" ]; then
...
DF_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 24)
info "Password: $DF_PASS"
...
section "Step 7: Starting DragonflyDB"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^dragonfly$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs dragonfly"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for DragonflyDB on TCP port 6382..."
HEALTH_OK=0
for i in $(seq 1 24); do
    if nc -z 127.0.0.1 6382 2>/dev/null; then
        info "Port 6382 is open — DragonflyDB is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 6382 not responding yet. Check: docker logs dragonfly"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🐉  DragonflyDB (Redis-compatible, multithreaded):  $SERVER_IP:6382"
echo "  ║  🔑  Password: $DF_PASS"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
