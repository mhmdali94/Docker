...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Semaphore UI Auto-Installer"
...
SERVICE_DIR="/root/docker/semaphore"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(semaphore)$' || true)
if [ -n "$EXISTING" ]; then
...
chown -R 1001:0 "$SERVICE_DIR/data"
...
section "Step 7: Starting Semaphore UI"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^semaphore$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs semaphore"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Semaphore UI to be ready on port 3021..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3021 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3021 is responding (HTTP $STATUS) — Semaphore UI is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3021 2>/dev/null; then
        warn "Port 3021 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3021 is NOT responding yet."
        docker logs --tail 20 semaphore 2>&1 || true
    fi
    warn "Check logs: docker logs semaphore"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🗂️   Semaphore (Ansible/Terraform UI):  http://$SERVER_IP:3021"
