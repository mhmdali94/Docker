...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║     Home Assistant Auto-Installer               ║"
...
echo ""
echo "  Home Assistant needs your timezone for correct scheduling and automations."
echo "  Examples: America/New_York, Europe/London, Asia/Dubai, Africa/Cairo"
echo ""
read -rp "  Enter your timezone [UTC]: " TZ_INPUT
TZ_INPUT="${TZ_INPUT:-UTC}"
info "Timezone: $TZ_INPUT"

section "Step 5: Cleaning Up Existing Containers"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E "^homeassistant$" || true)
[ -n "$EXISTING" ] && warn "Removing existing container..." && docker rm -f homeassistant 2>/dev/null || true

section "Step 6: Preparing Directory"
APP_DIR="/root/docker/homeassistant"
...
section "Step 8: Starting Home Assistant"
if docker compose version &> /dev/null; then
...
echo "  ║                                                      ║"
echo "  ║  📝  First visit: create your owner account         ║"
echo "  ║                                                      ║"
echo "  ║  📁  Config: $APP_DIR/config/"
echo "  ║      configuration.yaml — main config               ║"
echo "  ║      automations.yaml   — automations               ║"
echo "  ║                                                      ║"
echo "  ║  🔌  Uses host network — can discover LAN devices   ║"
echo "  ║                                                      ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️            ║"
echo "  ║       Made by: Mohammed Ali Elshikh                 ║"
echo "  ║       prismatechwork.com                            ║"
