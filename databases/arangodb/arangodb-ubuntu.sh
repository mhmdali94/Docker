...
SERVICE_DIR="/root/docker/arangodb"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(arangodb)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/data" "$SERVICE_DIR/apps"
...
section "Step 7: Starting ArangoDB"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^arangodb$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs arangodb"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for ArangoDB to be ready on port 8529..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8529 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8529 is responding (HTTP $STATUS) — ArangoDB is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8529 2>/dev/null; then
        warn "Port 8529 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8529 is NOT responding yet."
        docker logs --tail 20 arangodb 2>&1 || true
    fi
    warn "Check logs: docker logs arangodb"
fi
