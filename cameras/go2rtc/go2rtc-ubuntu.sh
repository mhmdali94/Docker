...
SERVICE_DIR="/root/docker/go2rtc"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(go2rtc)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting go2rtc"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^go2rtc$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs go2rtc"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for go2rtc to be ready on port 1984..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:1984 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 1984 is responding (HTTP $STATUS) — go2rtc is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 1984 2>/dev/null; then
        warn "Port 1984 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 1984 is NOT responding yet."
        docker logs --tail 20 go2rtc 2>&1 || true
    fi
    warn "Check logs: docker logs go2rtc"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🎥  go2rtc (camera streaming hub):  http://$SERVER_IP:1984"
echo "  ║  🔧  Add stream sources in the web UI or $SERVICE_DIR/config/go2rtc.yaml."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
