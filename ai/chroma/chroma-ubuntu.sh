...
SERVICE_DIR="/root/docker/chroma"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(chroma)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Chroma"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^chroma$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs chroma"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Chroma to be ready on port 8007..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8007/api/v2/heartbeat 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8007 is responding (HTTP $STATUS) — Chroma is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8007 2>/dev/null; then
        warn "Port 8007 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8007 is NOT responding yet."
        docker logs --tail 20 chroma 2>&1 || true
    fi
    warn "Check logs: docker logs chroma"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🧲  Chroma (vector DB):  http://$SERVER_IP:8007"
echo "  ║  🔗  API v2 heartbeat: http://$SERVER_IP:8007/api/v2/heartbeat"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
