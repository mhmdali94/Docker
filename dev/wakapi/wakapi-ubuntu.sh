...
SERVICE_DIR="/root/docker/wakapi"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(wakapi)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Wakapi"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^wakapi$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs wakapi"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Wakapi to be ready on port 3056..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3056 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3056 is responding (HTTP $STATUS) — Wakapi is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3056 2>/dev/null; then
        warn "Port 3056 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3056 is NOT responding yet."
        docker logs --tail 20 wakapi 2>&1 || true
    fi
    warn "Check logs: docker logs wakapi"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⌨️   Wakapi (coding statistics):  http://$SERVER_IP:3056"
echo "  ║  🔧  Register, grab your API key, and point your WakaTime plugin at this server."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
