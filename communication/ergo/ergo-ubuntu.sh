#!/bin/bash
# ============================================================
#   Ergo IRC Server Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Ergo IRC Server Auto-Installer"
...
SERVICE_DIR="/root/docker/ergo"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(ergo)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Ergo IRC Server"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^ergo$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs ergo"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Ergo IRC Server on TCP port 6667..."
HEALTH_OK=0
for i in $(seq 1 24); do
    if nc -z 127.0.0.1 6667 2>/dev/null; then
        info "Port 6667 is open — Ergo IRC Server is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 6667 not responding yet. Check: docker logs ergo"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  💬  Ergo IRC server:  $SERVER_IP:6667 (plain) / 6697 (TLS)"
echo "  ║  🔗  Pairs with The Lounge — add this server in its connect screen."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
