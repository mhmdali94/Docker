...
SERVICE_DIR="/root/docker/2fauth"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(2fauth)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting 2FAuth"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^2fauth$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs 2fauth"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for 2FAuth to be ready on port 8248..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8248 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8248 is responding (HTTP $STATUS) — 2FAuth is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8248 2>/dev/null; then
        warn "Port 8248 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8248 is NOT responding yet."
        docker logs --tail 20 2fauth 2>&1 || true
    fi
    warn "Check logs: docker logs 2fauth"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔢  2FAuth (web TOTP authenticator):  http://$SERVER_IP:8248"
