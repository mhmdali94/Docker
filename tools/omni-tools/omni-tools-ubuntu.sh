...
SERVICE_DIR="/root/docker/omni-tools"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(omni-tools)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting OmniTools"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^omni-tools$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs omni-tools"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for OmniTools to be ready on port 8176..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8176 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8176 is responding (HTTP $STATUS) — OmniTools is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8176 2>/dev/null; then
        warn "Port 8176 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8176 is NOT responding yet."
        docker logs --tail 20 omni-tools 2>&1 || true
    fi
    warn "Check logs: docker logs omni-tools"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🧰  OmniTools (utility toolbox):  http://$SERVER_IP:8176"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
