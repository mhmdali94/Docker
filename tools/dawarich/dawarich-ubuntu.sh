...
SERVICE_DIR="/root/docker/dawarich"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(dawarich|dawarich-sidekiq|dawarich-db|dawarich-redis)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/public" "$SERVICE_DIR/watched"
...
services:
  ...
  dawarich:
    ...
    environment: &dawarich-env
      ...
      APPLICATION_HOSTS: $SERVER_IP,localhost
...
section "Step 7: Starting Dawarich"
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^dawarich$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs dawarich"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Dawarich to be ready on port 3057..."
HEALTH_OK=0
for i in $(seq 1 48); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3057 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3057 is responding (HTTP $STATUS) — Dawarich is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3057 2>/dev/null; then
        warn "Port 3057 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3057 is NOT responding yet."
        docker logs --tail 20 dawarich 2>&1 || true
    fi
    warn "Check logs: docker logs dawarich"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🗺️   Dawarich (location history):  http://$SERVER_IP:3057"
echo "  ║  🔑  Default login: demo@dawarich.app / password  (change it!)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
