...
SERVICE_DIR="/root/docker/memcached"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(memcached)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Memcached"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^memcached$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs memcached"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Memcached on TCP port 11211..."
HEALTH_OK=0
for i in $(seq 1 24); do
    if nc -z 127.0.0.1 11211 2>/dev/null; then
        info "Port 11211 is open — Memcached is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 11211 not responding yet. Check: docker logs memcached"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🧠  Memcached:  $SERVER_IP:11211  (256 MB cache)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
