...
SERVICE_DIR="/root/docker/wizarr"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(wizarr)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Wizarr"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^wizarr$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs wizarr"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Wizarr to be ready on port 5690..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:5690 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 5690 is responding (HTTP $STATUS) — Wizarr is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 5690 2>/dev/null; then
        warn "Port 5690 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 5690 is NOT responding yet."
        docker logs --tail 20 wizarr 2>&1 || true
    fi
    warn "Check logs: docker logs wizarr"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🪄  Wizarr (media server invites):  http://$SERVER_IP:5690"
echo "  ║  🔧  Connect Jellyfin/Plex/Emby in the setup wizard, then send invite links."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
