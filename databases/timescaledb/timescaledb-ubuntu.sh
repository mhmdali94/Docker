...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   TimescaleDB Auto-Installer"
...
SERVICE_DIR="/root/docker/timescaledb"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(timescaledb)$' || true)
if [ -n "$EXISTING" ]; then
...
info "postgres password: $DB_PASS"
...
section "Step 7: Starting TimescaleDB"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^timescaledb$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs timescaledb"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for TimescaleDB on TCP port 5433..."
HEALTH_OK=0
for i in $(seq 1 24); do
    if nc -z 127.0.0.1 5433 2>/dev/null; then
        info "Port 5433 is open — TimescaleDB is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 5433 not responding yet. Check: docker logs timescaledb"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  ⏱️   TimescaleDB (PostgreSQL + time-series):  $SERVER_IP:5433"
echo "  ║  🔑  Login: postgres / $DB_PASS"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
