...
SERVICE_DIR="/root/docker/wekan"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(wekan|wekan-mongo)$' || true)
if [ -n "$EXISTING" ]; then
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^wekan$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs wekan"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for WeKan to be ready on port 8139..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8139 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8139 is responding (HTTP $STATUS) — WeKan is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8139 2>/dev/null; then
        warn "Port 8139 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8139 is NOT responding yet."
        docker logs --tail 20 wekan 2>&1 || true
    fi
    warn "Check logs: docker logs wekan"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📋  WeKan (kanban):  http://$SERVER_IP:8139"
echo "  ║  🔧  Register the first user — it becomes admin."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
