...
for cname in leantime leantime-db; do
...
APP_DIR="/root/docker/leantime"
mkdir -p "$APP_DIR"/{userfiles,public_userfiles}
...
services:
  ...
  leantime:
    ...
    environment:
      ...
      LEAN_APP_URL: http://${SERVER_IP}:8105
    volumes:
...
section "Step 7: Starting Leantime"
MAX_RETRIES=3
...
info "Waiting for Leantime on port 8105..."
HEALTH_OK=0
for i in $(seq 1 12); do
    if curl -s --max-time 3 http://127.0.0.1:8105 &>/dev/null; then
        info "Port 8105 is responding. ✅"; HEALTH_OK=1; break
    fi
    echo -n "  Attempt $i/12 — waiting 5s..."; sleep 5; echo " retrying"
done
[ "$HEALTH_OK" -eq 0 ] && warn "Not responding. Check: docker logs leantime"

section "Step 9: Opening Firewall"
if command -v ufw &>/dev/null; then
    ufw allow 8105/tcp; info "UFW: port 8105 opened."
else
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📋  Leantime (Project Management):                ║"
echo "  ║      👉  http://$SERVER_IP:8105"
echo "  ║                                                      ║"
