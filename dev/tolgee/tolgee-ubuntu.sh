...
SERVICE_DIR="/root/docker/tolgee"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(tolgee)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  tolgee:
    ...
    environment:
      ...
      TOLGEE_AUTHENTICATION_INITIAL_PASSWORD: $ADMIN_PASS
...
section "Step 7: Starting Tolgee"
warn "First start boots the embedded Postgres — allow a couple of minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^tolgee$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs tolgee"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Tolgee to be ready on port 8212..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8212 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8212 is responding (HTTP $STATUS) — Tolgee is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8212 2>/dev/null; then
        warn "Port 8212 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8212 is NOT responding yet."
        docker logs --tail 20 tolgee 2>&1 || true
    fi
    warn "Check logs: docker logs tolgee"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🌍  Tolgee (localization platform):  http://$SERVER_IP:8212"
