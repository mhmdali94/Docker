...
SERVICE_DIR="/root/docker/rabbitmq"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(rabbitmq)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting RabbitMQ"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^rabbitmq$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs rabbitmq"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for RabbitMQ to be ready on port 15672..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:15672 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 15672 is responding (HTTP $STATUS) — RabbitMQ is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 15672 2>/dev/null; then
        warn "Port 15672 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 15672 is NOT responding yet."
        docker logs --tail 20 rabbitmq 2>&1 || true
    fi
    warn "Check logs: docker logs rabbitmq"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🐰  RabbitMQ AMQP:  $SERVER_IP:5672"
echo "  ║  🖥️   Management UI:  http://$SERVER_IP:15672"
