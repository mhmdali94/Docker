...
section "Step 8: Starting Cachet"
if docker compose version &> /dev/null; then
...
info "Waiting for Cachet to be ready (~60s)..."
for i in $(seq 1 18); do
    if curl -sf --max-time 5 http://127.0.0.1:8099 &>/dev/null; then
        info "Cachet is ready. ✅"
        break
    fi
