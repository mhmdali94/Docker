...
SERVICE_DIR="/root/docker/milvus"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(milvus|milvus-etcd|milvus-minio)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/etcd" "$SERVICE_DIR/minio" "$SERVICE_DIR/milvus"
...
section "Step 7: Starting Milvus"
warn "Milvus standalone starts etcd + MinIO + Milvus — allow a few minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^milvus$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs milvus"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Milvus on TCP port 19530..."
HEALTH_OK=0
for i in $(seq 1 36); do
    if nc -z 127.0.0.1 19530 2>/dev/null; then
        info "Port 19530 is open — Milvus is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 19530 not responding yet. Check: docker logs milvus"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🧲  Milvus (vector database):  $SERVER_IP:19530"
echo "  ║  🔗  Connect with pymilvus: MilvusClient(uri="http://$SERVER_IP:19530")"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
