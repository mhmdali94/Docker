...
SERVICE_DIR="/root/docker/unleash"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(unleash|unleash-db)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Unleash"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^unleash$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs unleash"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Unleash to be ready on port 4242..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:4242 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 4242 is responding (HTTP $STATUS) — Unleash is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 4242 2>/dev/null; then
        warn "Port 4242 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 4242 is NOT responding yet."
        docker logs --tail 20 unleash 2>&1 || true
    fi
    warn "Check logs: docker logs unleash"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🚩  Unleash (feature flags):  http://$SERVER_IP:4242"
echo "  ║  🔑  Default login: admin / unleash4all  (change it!)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
