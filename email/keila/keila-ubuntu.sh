...
SERVICE_DIR="/root/docker/keila"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(keila|keila-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  keila:
    ...
    environment:
      ...
      DB_URL: postgres://keila:$DB_PASS@keila-db/keila
      URL_HOST: $SERVER_IP
      URL_PORT: 8227
      URL_SCHEMA: http
      KEILA_USER: admin@example.com
      KEILA_PASSWORD: $ADMIN_PASS
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^keila$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs keila"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Keila to be ready on port 8227..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8227 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8227 is responding (HTTP $STATUS) — Keila is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8227 2>/dev/null; then
        warn "Port 8227 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8227 is NOT responding yet."
        docker logs --tail 20 keila 2>&1 || true
    fi
    warn "Check logs: docker logs keila"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📮  Keila (newsletters):  http://$SERVER_IP:8227"
