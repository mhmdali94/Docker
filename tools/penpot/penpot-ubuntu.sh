...
SERVICE_DIR="/root/docker/penpot"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(penpot-frontend|penpot-backend|penpot-exporter|penpot-db|penpot-redis)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/assets"
...
services:
  ...
  penpot-backend:
    ...
    environment:
      ...
      PENPOT_PUBLIC_URI: http://$SERVER_IP:9009
...
section "Step 7: Starting Penpot"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^penpot-frontend$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs penpot-frontend"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Penpot to be ready on port 9009..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:9009 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 9009 is responding (HTTP $STATUS) — Penpot is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 9009 2>/dev/null; then
        warn "Port 9009 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 9009 is NOT responding yet."
        docker logs --tail 20 penpot-frontend 2>&1 || true
    fi
    warn "Check logs: docker logs penpot-frontend"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🎨  Penpot (open-source Figma):  http://$SERVER_IP:9009"
echo "  ║  🔑  Register your account on first visit (email verification disabled)."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
