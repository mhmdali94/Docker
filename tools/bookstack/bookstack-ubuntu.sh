...
for cname in bookstack bookstack-db; do
...
BS_DIR="/root/docker/bookstack"
if [ -d "$BS_DIR" ]; then
    warn "Removing old directory $BS_DIR..."
    rm -rf "$BS_DIR"
fi
mkdir -p "$BS_DIR"
cd "$BS_DIR" || error "Cannot navigate to $BS_DIR"
info "Directory ready: $BS_DIR"

section "Step 6: Generating Credentials & docker-compose.yml"
SERVER_IP=$(hostname -I | tr ' ' '\n' | grep -E '^[0-9]+\.' | head -1)
DB_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 20)
ROOT_PASS=$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 20)
info "Default Admin  : admin@admin.com"
info "Default Password: password"
warn "Change default credentials after first login!"

cat > "$BS_DIR/docker-compose.yml" <<EOF
services:
  bookstack-db:
    image: mariadb:10.11
...
section "Step 7: Starting BookStack"
MAX_RETRIES=3
...
sleep 15
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^bookstack$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs bookstack"
else
...
if [ "$HEALTH_OK" -eq 0 ]; then
    warn "BookStack may still be initializing. Check: docker logs bookstack"
fi

section "Step 10: Opening Firewall Port 6875"
if command -v ufw &> /dev/null; then
    ufw allow 6875/tcp
    info "UFW: port 6875/tcp opened."
else
...
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║                                                      ║"
echo "  ║  🌐  Open BookStack in your browser:              ║"
echo "  ║      👉  http://$SERVER_IP:6875"
echo "  ║                                                      ║"
