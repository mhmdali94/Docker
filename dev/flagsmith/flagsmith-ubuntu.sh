...
SERVICE_DIR="/root/docker/flagsmith"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(flagsmith|flagsmith-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  flagsmith:
    ...
    ports:
      - "8211:8000"
    environment:
      DATABASE_URL: postgresql://flagsmith:$DB_PASS@flagsmith-db:5432/flagsmith
...
section "Step 7: Starting Flagsmith"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^flagsmith$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs flagsmith"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Flagsmith to be ready on port 8211..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8211 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8211 is responding (HTTP $STATUS) — Flagsmith is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8211 2>/dev/null; then
        warn "Port 8211 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8211 is NOT responding yet."
        docker logs --tail 20 flagsmith 2>&1 || true
    fi
    warn "Check logs: docker logs flagsmith"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🚩  Flagsmith (feature flags):  http://$SERVER_IP:8211"
