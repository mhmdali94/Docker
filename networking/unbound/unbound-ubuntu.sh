...
SERVICE_DIR="/root/docker/unbound"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(unbound)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Unbound"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^unbound$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs unbound"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Unbound on TCP port 5335..."
HEALTH_OK=0
for i in $(seq 1 24); do
    if nc -z 127.0.0.1 5335 2>/dev/null; then
        info "Port 5335 is open — Unbound is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 5335 not responding yet. Check: docker logs unbound"

echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🌐  Unbound (recursive DNS resolver):  $SERVER_IP:5335"
echo "  ║  🔗  Point Pi-hole/AdGuard upstream DNS to $SERVER_IP#5335 for full independence from public resolvers."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
