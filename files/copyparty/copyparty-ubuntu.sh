...
SERVICE_DIR="/root/docker/copyparty"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(copyparty)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  copyparty:
    ...
    command: -a admin:$ADMIN_PASS -v /w:/:rw,admin --no-crt
    ports:
      - "3923:3923"
    volumes:
      - ./data:/w
...
section "Step 7: Starting copyparty"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^copyparty$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs copyparty"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for copyparty to be ready on port 3923..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3923 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3923 is responding (HTTP $STATUS) — copyparty is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3923 2>/dev/null; then
        warn "Port 3923 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3923 is NOT responding yet."
        docker logs --tail 20 copyparty 2>&1 || true
    fi
    warn "Check logs: docker logs copyparty"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🎉  copyparty (file server):  http://$SERVER_IP:3923"
