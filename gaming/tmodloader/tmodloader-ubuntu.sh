#!/bin/bash
# ============================================================
#   tModLoader Server Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   tModLoader Server Auto-Installer"
...
SERVICE_DIR="/root/docker/tmodloader"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(tmodloader)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting tModLoader Server"
warn "Downloading tModLoader — first start takes several minutes."
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^tmodloader$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs tmodloader"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for tModLoader Server on TCP port 7778..."
HEALTH_OK=0
for i in $(seq 1 48); do
    if nc -z 127.0.0.1 7778 2>/dev/null; then
        info "Port 7778 is open — tModLoader Server is up. ✅"
        HEALTH_OK=1
        break
    fi
...
[ "$HEALTH_OK" -eq 0 ] && warn "Port 7778 not responding yet. Check: docker logs tmodloader"
