...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   PrivateBin Auto-Installer"
...
SERVICE_DIR="/root/docker/privatebin"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(privatebin)$' || true)
if [ -n "$EXISTING" ]; then
...
chown -R 65534:82 "$SERVICE_DIR/data"
...
section "Step 7: Starting PrivateBin"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^privatebin$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs privatebin"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for PrivateBin to be ready on port 8111..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8111 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8111 is responding (HTTP $STATUS) — PrivateBin is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8111 2>/dev/null; then
        warn "Port 8111 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8111 is NOT responding yet."
        docker logs --tail 20 privatebin 2>&1 || true
    fi
    warn "Check logs: docker logs privatebin"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔒  PrivateBin (encrypted pastebin):  http://$SERVER_IP:8111"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
