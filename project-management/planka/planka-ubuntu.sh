...
SERVICE_DIR="/root/docker/planka"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(planka|planka-db)$' || true)
if [ -n "$EXISTING" ]; then
...
services:
  ...
  planka:
    ...
    ports:
      - "3061:1337"
    environment:
      BASE_URL: http://$SERVER_IP:3061
      DATABASE_URL: postgresql://planka:$DB_PASS@planka-db/planka
...
section "Step 7: Starting Planka"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^planka$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs planka"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Planka to be ready on port 3061..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3061 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3061 is responding (HTTP $STATUS) — Planka is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3061 2>/dev/null; then
        warn "Port 3061 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3061 is NOT responding yet."
        docker logs --tail 20 planka 2>&1 || true
    fi
    warn "Check logs: docker logs planka"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📋  Planka (Trello alternative):  http://$SERVER_IP:3061"
