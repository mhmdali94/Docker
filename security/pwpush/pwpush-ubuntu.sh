#!/bin/bash
# ============================================================
#   Password Pusher Auto-Installer
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   Password Pusher Auto-Installer"
...
SERVICE_DIR="/root/docker/pwpush"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(pwpush)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/storage"
...
section "Step 7: Starting Password Pusher"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^pwpush$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs pwpush"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Password Pusher to be ready on port 8174..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8174 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8174 is responding (HTTP $STATUS) — Password Pusher is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8174 2>/dev/null; then
        warn "Port 8174 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8174 is NOT responding yet."
        docker logs --tail 20 pwpush 2>&1 || true
    fi
    warn "Check logs: docker logs pwpush"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🔐  Password Pusher:  http://$SERVER_IP:8174"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
