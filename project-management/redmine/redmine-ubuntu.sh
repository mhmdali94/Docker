...
SERVICE_DIR="/root/docker/redmine"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(redmine|redmine-db)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Redmine"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^redmine$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs redmine"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Redmine to be ready on port 3016..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3016 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 3016 is responding (HTTP $STATUS) — Redmine is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 3016 2>/dev/null; then
        warn "Port 3016 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 3016 is NOT responding yet."
        docker logs --tail 20 redmine 2>&1 || true
    fi
    warn "Check logs: docker logs redmine"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  🗂️   Redmine:  http://$SERVER_IP:3016"
echo "  ║  🔑  Default login: admin / admin  (you must change it on first login)"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
