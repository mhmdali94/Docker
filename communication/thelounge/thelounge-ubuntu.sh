...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║   The Lounge Auto-Installer"
...
SERVICE_DIR="/root/docker/thelounge"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(thelounge)$' || true)
if [ -n "$EXISTING" ]; then
...
info "Suggested user password: $USER_PASS"
...
section "Step 7: Starting The Lounge"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^thelounge$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs thelounge"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for The Lounge to be ready on port 9020..."
HEALTH_OK=0
for i in $(seq 1 24); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:9020 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 9020 is responding (HTTP $STATUS) — The Lounge is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 9020 2>/dev/null; then
        warn "Port 9020 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 9020 is NOT responding yet."
        docker logs --tail 20 thelounge 2>&1 || true
    fi
    warn "Check logs: docker logs thelounge"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  💬  The Lounge (web IRC client):  http://$SERVER_IP:9020"
echo "  ║  ➕  Create your user:"
echo "  ║      docker exec -it thelounge thelounge add admin"
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
