...
section "Step 8: Starting osTicket"
if docker compose version &> /dev/null; then
...
info "Waiting for osTicket to be ready..."
for i in $(seq 1 18); do
    if curl -sf --max-time 5 http://127.0.0.1:8088 &>/dev/null; then
        info "osTicket is ready. ✅"
        break
    fi
