...
SERVICE_DIR="/root/docker/flarum"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(flarum|flarum-db)$' || true)
if [ -n "$EXISTING" ]; then
...
mkdir -p "$SERVICE_DIR" "$SERVICE_DIR/db" "$SERVICE_DIR/assets" "$SERVICE_DIR/extensions"
...
services:
  ...
  flarum:
    ...
    environment:
      ...
      FLARUM_ADMIN_MAIL: admin@localhost.local
...
section "Step 7: Starting Flarum"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^flarum$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs flarum"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Flarum to be ready on port 8164..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:8164 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 8164 is responding (HTTP $STATUS) — Flarum is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 8164 2>/dev/null; then
        warn "Port 8164 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 8164 is NOT responding yet."
        docker logs --tail 20 flarum 2>&1 || true
    fi
    warn "Check logs: docker logs flarum"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  💬  Flarum (forum):  http://$SERVER_IP:8164"
