...
SERVICE_DIR="/root/docker/scrypted"
EXISTING=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^(scrypted)$' || true)
if [ -n "$EXISTING" ]; then
...
section "Step 7: Starting Scrypted"
MAX_RETRIES=3
...
sleep 10
RUNNING=$(docker ps --format '{{.Names}}' | grep -E '^scrypted$' || true)
if [ -z "$RUNNING" ]; then
    warn "Container may not have started. Check: docker logs scrypted"
else
    info "Container running: $RUNNING"
fi

section "Step 10: Health Check"
info "Waiting for Scrypted to be ready on port 10443..."
HEALTH_OK=0
for i in $(seq 1 36); do
    STATUS=$(curl -s -k -o /dev/null -w "%{http_code}" --max-time 5 https://127.0.0.1:10443 2>/dev/null || echo "000")
    if echo "$STATUS" | grep -qE '^(200|301|302|303|401)$'; then
        info "Port 10443 is responding (HTTP $STATUS) — Scrypted is healthy. ✅"
        HEALTH_OK=1
        break
    fi
...
if [ "$HEALTH_OK" -eq 0 ]; then
    if nc -z 127.0.0.1 10443 2>/dev/null; then
        warn "Port 10443 is open but HTTP did not respond. Service may still be starting."
    else
        warn "Port 10443 is NOT responding yet."
        docker logs --tail 20 scrypted 2>&1 || true
    fi
    warn "Check logs: docker logs scrypted"
fi
...
echo "  ║              ✅  Setup Complete!                     ║"
echo "  ╠══════════════════════════════════════════════════════╣"
echo "  ║  📹  Scrypted (camera hub):  https://$SERVER_IP:10443"
echo "  ║  🔧  Create the admin account on first visit."
echo "  ║  🍏  Bridges cameras to HomeKit, Google Home and Alexa with low latency."
echo "  ║"
echo "  ║  ⚠️  FOR DEMO / TESTING PURPOSES ONLY ⚠️"
echo "  ║       Made by: Mohammed Ali Elshikh"
echo "  ║       prismatechwork.com"
